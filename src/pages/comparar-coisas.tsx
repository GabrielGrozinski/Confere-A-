    ...
    return (
        <div style={{ background: dark ? "linear-gradient(to bottom right, #0d1015, #080c14)" : "linear-gradient(to bottom right, #f7fbff, #fdfeff)"}}>
            ...
            <div className="min-h-screen grid grid-cols-[auto_1fr_auto] lg:px-4 items-center pt-2 pb-10 relative">
                ...
                <main className={`col-span-full lg:col-2 w-full flex flex-col mt-24 relative pt-2 ${(assinanteAtual !== 'Sócio' && assinanteAtual !== 'Torcedor') ? 'lg:max-w-250 lg:min-w-250' : 'lg:min-w-[80%] lg:max-w-[80%] lg:translate-x-[10vw]'}`}>

                    <div className="rounded-lg rounded-t-none min-h-115 max-h-115 grid grid-rows-[1fr_15%]">

                        <main id="main" className={`grid grid-cols-[1fr_auto_1fr] relative mx-1 border max-h-100 scale-90 lg:scale-100 ${dark ? 'bg-[#3e495e] border-slate-800/20' : 'border-slate-400/20 bg-slate-100'}`}>
                            ...
                            <div id="clubes" className="pt-4 flex flex-col lg:flex-row lg:flex-wrap lg:justify-between lg:items-start lg:px-4 lg:content-start items-center gap-4 justify-start p-2 overflow-y-auto pb-4">
                                {clubesSelecionados.map((clube, index) => {
                                    ...
                                    return (
                                    <div key={index} className={`flex flex-col sm:flex-row pb-3 sm:max-h-18.5 sm:min-h-18.5 min-h-27.5 max-h-28.5 items-center relative px-1 justify-evenly py-2 rounded-md min-w-[90%] max-w-[90%] lg:max-w-[47.5%] lg:min-w-[47.5%] ${dark ? 'bg-[#32394a]' : 'bg-slate-200'}`}>
                                        <img className="max-h-10 max-w-10 sm:max-w-12 sm:max-h-12" src={clube.imagem} alt={clube.nome} />
                                        <div className="flex flex-col gap-0.5 text-sm sm:ml-2 items-center sm:items-start">
                            ...
                            <div id="coisas" className="col-3 pt-4 flex flex-col lg:flex-row lg:flex-wrap lg:justify-between lg:items-start lg:px-4 lg:content-start items-center gap-4 justify-start p-2 relative overflow-y-auto pb-4">
                                {coisas?.map((coisa) => {
                                    ...
                                    return (
                                    coisa.quantidade > 0 &&
                                    <div key={coisa.id} className={`flex flex-col sm:flex-row sm:max-h-20 sm:min-h-20 min-h-27.5 max-h-28.5 pt-3 items-center relative pb-3 px-1 justify-between rounded-md min-w-[90%] max-w-[90%] lg:max-w-[47.5%] lg:min-w-[47.5%] ${dark ? 'bg-[#32394a]' : 'bg-slate-200'}`}>
                                        <img className="max-h-14 max-w-[40%] pl-1 sm:pl-4" src={coisa.imagem} alt={coisa.nome} />
                                        <div className="flex flex-col gap-0.5 sm:ml-2 min-w-[90%] sm:min-w-[40%] sm:max-w-[40%] text-center items-center sm:items-start">
                                            <h1 className={`font-medium min-w-full text-xs ${dark ? 'text-slate-100' : 'text-zinc-800'}`}>{coisa.nome}</h1>
                                        ...
                                        <span className={`absolute bottom-0 left-0 min-w-[75%] max-h-2 min-h-1.5 rounded-xl ${dark ? 'bg-slate-800' : 'bg-slate-300'}`}>
                                            ...
                                            <span className={`absolute bottom-0 -right-[25%] text-shadow-[1px_1px_1px_#000002a] text-[10px] translate-x-1/5 ${dark ? 'text-sky-100' : 'text-sky-900'}`}>
                                                {coisa.quantidade}x
                                            </span>
                                        </span>
                                    </div>
                            ...
                            <div className={`absolute z-1 bottom-0 translate-y-full left-0 min-h-6 min-w-[calc(100%+1.85px)] border -translate-x-px p-2 grid grid-cols-[1fr_auto_1fr] items-center ${dark ? 'border-slate-800/40 bg-[#3e495e]' : 'bg-slate-100 border-slate-400/20'}`}>
