...
export function InfoCard(
    ...
    return (
        ...
        className={`cursor-pointer relative ${dark ? 'bg-[#1a1625]' : 'bg-sky-50'} shadow-[1px_1px_3px_#0000002a] rounded-2xl flex flex-col justify-center pl-4 pr-2 py-2 gap-1 ${largura > 768 ? 'min-h-32 max-h-32' : 'min-h-30 max-h-30'} lg: min-w-[calc(50%-24px)] overflow-hidden`}>
                    ...
                    <Area
                        ...
                        stroke={`
                            ${(() => {
                                ...
                                if (media.titulo === 'Valor Estimado') return '#60A5FA';
                        ...
                        fill={`
                            ${(() => {
                                ...
                                if (media.titulo === 'Valor Estimado') return '#60A5FA8a';
    ...
    useEffect(() => {
        if (clubeEscolhido && media && chanceTitulo) {
            ...
            const mediaClubeEscolhido: Medias = {
                ...
                valor_estimado: Math.round(
                    ((clubeEscolhido.valor_estimado / (media.valor_estimado ?? 1)) * 100 - 100) * 100
                ) / 100,
            }
            ...
            if (mediaClubeEscolhido.notaClube && mediaClubeEscolhido.chanceQuitarDivida) {
                const mediaCardData: MediaCardData[] = [
                    ...
                    { valor: mediaClubeEscolhido.valor_estimado, titulo: "Valor Estimado" },
                            ...
                            <div className="mt-12">
                                ...
                                <span className="contents">
                                    {planos.map((plano) => (
                                        <div
                                        key={plano.id}
                                        className={`rounded-xl grid max-h-180 min-h-180 grid-rows-[1fr_10%] bg-card text-card-foreground shadow relative border-2 hover:shadow-2xl transition-all duration-300 overflow-hidden group ${plano.id === 2 ? dark ? 'border-red-500 hover:border-red-600 bg-[rgb(26,28,30)]' : 'border-red-400 hover:border-red-500 bg-white' : dark ? 'border-amber-300 hover:border-amber-400 bg-[rgb(26,28,30)]' : 'border-yellow-200 hover:border-yellow-400 bg-white'}`}
                                        >
                                            <div>
